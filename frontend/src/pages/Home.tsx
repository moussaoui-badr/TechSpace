...
export function HomePage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-14 px-4 py-8 sm:px-6 lg:space-y-20 lg:py-12">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7">
          {NAV_CATEGORIES.map((cat) => (
            <Link
              ...
              className="group flex flex-col items-center gap-3 rounded-md border border-border bg-background p-4 text-center transition-all hover:-translate-y-0.5 hover:border-primary hover:shadow-md"
            >
              <span className="flex h-14 w-14 items-center justify-center rounded-full bg-primary/10 text-2xl transition-all group-hover:scale-110 group-hover:bg-primary/15">
                {cat.icon}
              </span>
      ...
      {/* 4. Bandeau promo */}
      <section
        className="relative overflow-hidden rounded-md border-l-4 border-primary bg-secondary p-8 text-white md:p-12"
      >
      ...
      {!loading && brands.length > 0 && (
        <section>
          ...
          />
          <div className="flex flex-wrap items-center justify-center gap-3 rounded-md border border-border bg-surface p-6 sm:gap-6 sm:p-8">
            {brands.map((b) => (
              <div
                key={b.id}
                className="flex h-14 w-28 items-center justify-center rounded-md border border-border bg-background px-4 text-sm font-bold text-text-muted transition-all hover:border-primary hover:text-text hover:shadow-sm sm:h-16 sm:w-36"
              >
                {b.name}
              </div>
            ))}
      ...
      <section>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {trustItems.map(({ icon: Icon, title, description }) => (
            <div
              key={title}
              className="group flex items-start gap-3 rounded-md border border-border bg-background p-5 transition-all hover:border-primary hover:shadow-md"
            >
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary transition-all group-hover:bg-primary group-hover:text-white">
      ...
      <section className="relative overflow-hidden rounded-md bg-accent p-8 md:p-12">
        <div className="relative z-10 grid gap-6 lg:grid-cols-[1.2fr_1fr] lg:items-center">
          <div>
            <span className="inline-flex items-center gap-1.5 rounded-sm bg-text px-2.5 py-1 text-xs font-bold uppercase tracking-wider text-accent">
